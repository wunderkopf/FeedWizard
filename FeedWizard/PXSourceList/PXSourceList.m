...
#define DISCLOSURE_TRIANGLE_SPACE				4.0		//The indentation reserved for disclosure triangles for non-group items
...
- (void)reloadData
{
	   ...
	   [_secondaryDelegate respondsToSelector:@selector(sourceList:isGroupAlwaysExpanded:)])
	{
		for (NSUInteger i=0; i < [self numberOfGroups]; i++)
		{
			...
			if ([self isGroupAlwaysExpanded:item] || [self isGroupExpanded:item]) {
...
- (BOOL)isGroupExpanded:(id)group
{
    //Make sure that the item IS a group to prevent unwanted queries sent to the data source
	if([self isGroupItem:group]) {
		//Query the data source
		if([_secondaryDelegate respondsToSelector:@selector(sourceList:isGroupExpanded:)]) {
			return [_secondaryDelegate sourceList:self isGroupExpanded:group];
		}
	}
